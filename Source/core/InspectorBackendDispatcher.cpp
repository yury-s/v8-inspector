...
const char InspectorBackendDispatcher::commandNames[] = {
    ...
    "Tracing.end\0"
    "Animation.enable\0"
    "Animation.getAnimationPlayersForNode\0"
...
public:
        ...
        , m_accessibilityAgent(0)
    {
        ...
        const CallHandler handlers[] = {
            ...
            &InspectorBackendDispatcherImpl::Animation_getAnimationPlayersForNode,
...
private:
    ...
    void CSS_setPropertyText(int callId, JSONObject* requestMessageObject, JSONArray* protocolErrors);
    ...
    void Animation_getAnimationPlayersForNode(int callId, JSONObject* requestMessageObject, JSONArray* protocolErrors);
...
void InspectorBackendDispatcherImpl::CSS_setPropertyText(int callId, JSONObject* requestMessageObject, JSONArray* protocolErrors)
{
    if (!m_cssAgent)
        protocolErrors->pushString("CSS handler is not available.");
...
void InspectorBackendDispatcherImpl::Animation_getAnimationPlayersForNode(int callId, JSONObject* requestMessageObject, JSONArray* protocolErrors)
{
    if (!m_animationAgent)
        protocolErrors->pushString("Animation handler is not available.");
    ...
    JSONObject* paramsContainerPtr = paramsContainer.get();
    int in_nodeId = getInt(paramsContainerPtr, "nodeId", 0, protocolErrors);
    bool in_includeSubtreeAnimations = getBoolean(paramsContainerPtr, "includeSubtreeAnimations", 0, protocolErrors);

    RefPtr<TypeBuilder::Array<TypeBuilder::Animation::AnimationPlayer> > out_animationPlayers;

    if (protocolErrors->length()) {
        reportProtocolError(callId, InvalidParams, String::format(InvalidParamsFormatString, commandName(kAnimation_getAnimationPlayersForNodeCmd)), protocolErrors);
        return;
    }
    ErrorString error;
    RefPtr<JSONObject> result = JSONObject::create();
    m_animationAgent->getAnimationPlayersForNode(&error, in_nodeId, in_includeSubtreeAnimations, out_animationPlayers);
    if (!error.length()) {
        result->setValue("animationPlayers", out_animationPlayers);
    }
    sendResponse(callId, error, result);
}
